"""
Implementation of query interface for ontology agent.
"""
import logging
...
def query_ontology(self, query: str) -> List[Dict[str, Any]]:
    ...
    elif any(term in query for term in ["method", "how to make", "prepare", "mix"]):
        # Query for preparation methods
        for cocktail in ["martini", "manhattan", "margarita", "mojito", "negroni"]:
            if cocktail in query:
                results.extend(self._query_method_for_cocktail(cocktail))
    
    elif any(term in query for term in ["description", "what is", "define"]):
        # Query for cocktail descriptions
        for cocktail in ["martini", "manhattan", "margarita", "mojito", "negroni"]:
            if cocktail in query:
                results.extend(self._query_description_for_cocktail(cocktail))
...
def _query_glass_for_cocktail(self, cocktail: str) -> List[Dict[str, Any]]:
    """Query for the type of glass used for a specific cocktail"""
    sparql_query = f"""
    SELECT ?glass
    WHERE {{
        ?c rdf:type cocktail:Cocktail .
        ?c property:servedIn ?glass .
        FILTER(CONTAINS(LCASE(STR(?c)), "{cocktail}"))
    }}
    """
    ...
    for row in qres:
        glass_uri = str(row.glass)
        glass_name = glass_uri.split('#')[-1]
        results.append({
            "type": "glass_for_cocktail",
            "cocktail": cocktail,
            "glass": glass_name
        })
...
def _query_method_for_cocktail(self, cocktail: str) -> List[Dict[str, Any]]:
    """Query for the preparation method of a specific cocktail"""
    sparql_query = f"""
    SELECT ?method
    WHERE {{
        ?c rdf:type cocktail:Cocktail .
        ?c property:preparedBy ?method .
        FILTER(CONTAINS(LCASE(STR(?c)), "{cocktail}"))
    }}
    """
    ...
    for row in qres:
        method_uri = str(row.method)
        method_name = method_uri.split('#')[-1]
        results.append({
            "type": "method_for_cocktail",
            "cocktail": cocktail,
            "method": method_name
        })
...
def _query_description_for_cocktail(self, cocktail: str) -> List[Dict[str, Any]]:
    """Query for the description of a specific cocktail"""
    sparql_query = f"""
    SELECT ?description
    WHERE {{
        ?c rdf:type cocktail:Cocktail .
